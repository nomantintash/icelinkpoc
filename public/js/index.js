...
fm.icelink.Util.addOnLoad(function () {
    ...
    var start = function (sessionId, name) {
        ...
        console.log(video)
        var videoData = video.getElementsByTagName('video');



           
            // video.appendChild(videoData[0]);
    ...
    fm.icelink.Util.observe(screencaptureCheckbox, 'click', function (evt) {
        if (this.checked) {
            if (navigator.webkitGetUserMedia && !fm.icelink.LocalMedia.getChromeExtensionInstalled()) {
